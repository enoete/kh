...
function imgHtml(story,h,idx){
  ...
  return story.image
    ...
    :'<div style="background:linear-gradient(135deg,'+c[0]+','+c[1]+');min-height:'+h+'px;width:100%;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,0.3);font-size:10px;letter-spacing:0.15em;text-transform:uppercase;">Image</div>';
}

function shareHtml(s){
  return '<button class="bm-share-btn" data-sid="'+s.id+'">↗ Share</button>'
    +'<div class="bm-share-panel" id="bsp-'+s.id+'">'
    +'<button class="bm-sopt" data-sid="'+s.id+'" data-p="facebook" style="color:#1877f2;">f Facebook</button>'
    +'<button class="bm-sopt" data-sid="'+s.id+'" data-p="twitter" style="color:#1da1f2;">X Twitter</button>'
    +'<button class="bm-sopt" data-sid="'+s.id+'" data-p="whatsapp" style="color:#25d366;">W WhatsApp</button>'
    +'<button class="bm-sopt" data-sid="'+s.id+'" data-p="email" style="color:#8b6f47;">@ Email</button>'
    +'<button class="bm-sopt" data-sid="'+s.id+'" data-p="copy" style="color:#1a1a18;">Copy Link</button>'
    +'</div>';
}
...
window.bmRender=function(filter){
  ...
  var gh=rest.length?'<div class="stories-grid">'+rest.map(function(s,i){
    ...
    return '<a class="story-card story-lnk" href="'+s.url+'" data-sid="'+s.id+'" data-category="'+s.category+'">'
      ...
      +'<div class="story-card-footer"><span>'+s.date+'</span><span>'+s.readTime+'</span>'
    ...
    btn.addEventListener("click",function(e){
      ...
      if(p==="copy"){
        navigator.clipboard.writeText(fu).then(function(){
          ...
          panel.innerHTML='<span style="font-size:11px;color:#27ae60;padding:0.4rem 0.8rem;">✓ Copied!</span>';
...
st.textContent='.bm-share-btn{background:none;border:1px solid #e2dbd2;color:#9a9088;font-family:\'Jost\',sans-serif;font-size:11px;letter-spacing:0.15em;text-transform:uppercase;padding:0.4rem 0.8rem;cursor:pointer;transition:all 0.2s;margin-top:0.8rem;display:inline-block;}.bm-share-btn:hover{border-color:#1a1a18;color:#1a1a18;}.bm-share-panel{display:none;flex-direction:column;gap:0.3rem;margin-top:0.3rem;background:#fff;border:1px solid #e2dbd2;padding:0.5rem;box-shadow:0 4px 20px rgba(0,0,0,0.1);min-width:180px;}.bm-share-panel.open{display:flex;}.bm-sopt{background:none;border:none;text-align:left;padding:0.4rem 0.6rem;font-family:\'Jost\',sans-serif;font-size:11px;letter-spacing:0.1em;cursor:pointer;width:100%;}.bm-sopt:hover{background:#f7f3ee;}';
